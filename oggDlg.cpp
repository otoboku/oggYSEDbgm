...
BOOL CAboutDlg::OnInitDialog()
{
	...
	s.Format(_T("Version %d.%d Build %d %s %s"),in.dwMajorVersion,in.dwMinorVersion,in.dwBuildNumber,ss,in.szCSDVersion);
...
void COggDlg::Modec(){
	...
	ret+=_chdir("bgm");
	if(ret!=0){d_ed6sc.EnableWindow(FALSE);m_ed6sc.EnableWindow(FALSE);}else{d_ed6sc.EnableWindow(TRUE);m_ed6sc.EnableWindow(TRUE);}
	ret=_tchdir(savedata.ed6fc);
	ret+=_chdir("bgm");
	if(ret!=0){d_ed6fc.EnableWindow(FALSE);m_ed6fc.EnableWindow(FALSE);}else{d_ed6fc.EnableWindow(TRUE);m_ed6fc.EnableWindow(TRUE);}
	ret=_tchdir(savedata.ysf);
	ret+=_chdir("RELEASE\\MUSIC");
	if(ret!=0){d_ys3.EnableWindow(FALSE);m_ysf.EnableWindow(FALSE);}else{d_ys3.EnableWindow(TRUE);m_ysf.EnableWindow(TRUE);}
	ret=_tchdir(savedata.ys6);
	ret+=_chdir("RELEASE\\MUSIC");
	if(ret!=0){d_ys6.EnableWindow(FALSE);m_ys6.EnableWindow(FALSE);}else{d_ys6.EnableWindow(TRUE);m_ys6.EnableWindow(TRUE);}
	ret=_tchdir(savedata.yso);
	ret+=_chdir("RELEASE\\MUSIC");
	if(ret!=0){d_yso.EnableWindow(FALSE);m_yso.EnableWindow(FALSE);}else{d_yso.EnableWindow(TRUE);m_yso.EnableWindow(TRUE);}
	ret=_tchdir(savedata.ed6tc);
	ret+=_chdir("bgm");
	if(ret!=0){d_ed6tc.EnableWindow(FALSE);m_ed6tc.EnableWindow(FALSE);}else{d_ed6tc.EnableWindow(TRUE);m_ed6tc.EnableWindow(TRUE);}
	ret=_tchdir(savedata.zweiii);
	ret+=_chdir("bgm");
	if(ret!=0){d_z2.EnableWindow(FALSE);m_zweiii.EnableWindow(FALSE);}else{d_z2.EnableWindow(TRUE);m_zweiii.EnableWindow(TRUE);}
	ret=_tchdir(savedata.ysc);
	ret+=_chdir("bgm\\ys1");
	if(ret!=0){d_ysc1.EnableWindow(FALSE);m_ysc1.EnableWindow(FALSE);}else{d_ysc1.EnableWindow(TRUE);m_ysc1.EnableWindow(TRUE);}
	ret=_tchdir(savedata.ysc);
	ret+=_chdir("bgm\\ys2");
	if(ret!=0){d_ysc2.EnableWindow(FALSE);m_ysc2.EnableWindow(FALSE);}else{d_ysc2.EnableWindow(TRUE);m_ysc2.EnableWindow(TRUE);}
	ret=_tchdir(savedata.xa);
	ret+=_chdir("data\\bgm");
	if(ret!=0){d_xa.EnableWindow(FALSE);m_xa.EnableWindow(FALSE);}else{d_xa.EnableWindow(TRUE);m_xa.EnableWindow(TRUE);}
	ret=_tchdir(savedata.ys12);
	d_ys1.EnableWindow(TRUE);m_ys121.EnableWindow(TRUE);
	if(_chdir("wave\\wave_44")==-1){
		if(_chdir("wave\\wave_22")==-1){d_ys1.EnableWindow(FALSE);m_ys121.EnableWindow(FALSE);}
	}
	ret=_tchdir(savedata.ys122);
	d_ys2.EnableWindow(TRUE);m_ys122.EnableWindow(TRUE);
	if(_chdir("wave\\wave_44")==-1){
		if(_chdir("wave\\wave_22")==-1){d_ys2.EnableWindow(FALSE);m_ys122.EnableWindow(FALSE);}
	}
	ret=_tchdir(savedata.sor);
	d_sor.EnableWindow(TRUE);m_sor.EnableWindow(TRUE);
	if(_chdir("WAVE\\WAVE44")==-1){
		if(_chdir("WAVE\\WAVE22")==-1){d_sor.EnableWindow(FALSE);m_sor.EnableWindow(FALSE);}
	}
	d_z1.EnableWindow(TRUE);m_zwei.EnableWindow(TRUE);
	ret=_tchdir(savedata.zwei);
	{	CFile f;if(f.Open(_T("wav.dat"),CFile::modeRead,NULL)!=TRUE){d_z1.EnableWindow(FALSE);m_zwei.EnableWindow(FALSE);} else f.Close(); }
	ret=_tchdir(savedata.gurumin);
	ret+=_chdir("bgm");
	if(ret!=0){d_guru.EnableWindow(FALSE);m_gurumin.EnableWindow(FALSE);}else{d_guru.EnableWindow(TRUE);m_gurumin.EnableWindow(TRUE);}
	d_dino.EnableWindow(TRUE);m_dino.EnableWindow(TRUE);
	ret=_tchdir(savedata.dino);
	{	CFile f;if(f.Open(_T("bgm.arc"),CFile::modeRead,NULL)!=TRUE) {d_dino.EnableWindow(FALSE);m_dino.EnableWindow(FALSE);} else f.Close(); }
	ret=_tchdir(savedata.br4);
	ret+=_chdir("wave");
	if(ret!=0){d_br4.EnableWindow(FALSE);m_br4.EnableWindow(FALSE);}else{d_br4.EnableWindow(TRUE);m_br4.EnableWindow(TRUE);}
	ret=_tchdir(savedata.ed3);
	ret+=_chdir("wave");
	if(ret!=0){d_ed3.EnableWindow(FALSE);m_ed3.EnableWindow(FALSE);}else{d_ed3.EnableWindow(TRUE);m_ed3.EnableWindow(TRUE);}
	d_ed4.EnableWindow(TRUE);m_ed4.EnableWindow(TRUE);
	ret=_tchdir(savedata.ed4);
	if(_chdir("WAVEDV")==-1){
		if(_chdir("WAVE")==-1){d_ed4.EnableWindow(FALSE);m_ed4.EnableWindow(FALSE);}
	}
	d_ed5.EnableWindow(TRUE);d_ed5.EnableWindow(TRUE);
	ret=_tchdir(savedata.ed5);
	if(_chdir("WAVEDVD")==-1){
		if(_chdir("WAVE")==-1){d_ed5.EnableWindow(FALSE);m_ed5.EnableWindow(FALSE);}
	}
	ret=_tchdir(savedata.tuki);
	ret+=_chdir("MUSIC");
	if(ret!=0){d_tuki.EnableWindow(FALSE);}else{d_tuki.EnableWindow(TRUE);}
	ret=_tchdir(savedata.nishi);
	ret+=_chdir("bgm");
	if(ret!=0){d_nishi.EnableWindow(FALSE);}else{d_nishi.EnableWindow(TRUE);}
	d_arc.EnableWindow(TRUE);
	ret=_tchdir(savedata.arc);
	{	CFile f;if(f.Open(_T("music.pak"),CFile::modeRead,NULL)!=TRUE) d_arc.EnableWindow(FALSE); else f.Close(); }
	ret=_tchdir(savedata.san1);
	ret+=_chdir("music");
	if(ret!=0){d_san1.EnableWindow(FALSE);}else{d_san1.EnableWindow(TRUE);}
...
void COggDlg::play()
{
	...
	if(mode==15){
		if(ret2==40){ filen="bgm01.de2";
		loop1=loop2=0;}
	}
	if(mode==16){
		if(ret2==33){ filen="dinow_01(bgm.arc)";
		loop1=loop2=0;}
	}
	if(mode==10){
	...
	if(ret!=0){fnn="ファイル又はフォルダがありません";filen=""; 
...
//		playwavds2(bufwav3,0,dwDataLen*4,0);
	...
	for(i=0; i<=88; i++,iii++) { // 低音域用
		...
		if( logtbl[i] > BUFSZ-1 ) logtbl[i] = BUFSZ-1;
	}
...
void CWread::wavread()
{
	...
	if(mode==10){
		...
		adbuf2= (char*)malloc((size_t)adpcmf.GetLength()*4);
...
*/	}else if(mode==-11  || (mode==-14 && filen.Right(3)=="mp3") || mode==-15){
		...
		data_size=oggsize=loop2*4;
		adbuf2=(char*)calloc(data_size+44100*4,1);
		if(san2==1) loop2=225*44100;
		if(san2==2) loop2=247*44100;
		loop3=loop2;
		if(san2)data_size=oggsize=loop2*4;
...
//			dcsub.FillSolidRect(0,0,3000,30,RGB(1,1,1));
		...
		//枠はみ出し時スクロール処理
			if(si>MDC){
				dc.BitBlt(8*5,0+80,88*2+170,16+80,&dcsub,mcnt6,0,SRCINVERT);
				if(si-mcnt6<MDC){
					mcnt5++;
					dc.BitBlt(MDC-mcnt5+8*5,0+80,88*2+170,16+80,&dcsub,0,0,SRCINVERT);
					if(MDC-mcnt5<=0){mcnt5=0;mcnt6=0;}
				}else mcnt5=0;
				mcnt6++;
			}else{
				dc.BitBlt(8*5,0+80,88*2+170,16+80,&dcsub,0,0,SRCINVERT);
			}
		}else{
			if(tcg<50)
...
//					fnn="ランダム演奏中";
				if(m_yso.GetCheck()==0 && m_ys6.GetCheck()==0 && m_ysf.GetCheck()==0 && m_ed6fc.GetCheck()==0 && m_ed6sc.GetCheck()==0&& m_ed6tc.GetCheck()==0&&m_zweiii.GetCheck()==0&&m_ysc1.GetCheck()==0&&m_ysc2.GetCheck()==0&&m_xa.GetCheck()==0&&m_ys121.GetCheck()==0&&m_ys122.GetCheck()==0&&m_sor.GetCheck()==0&&m_zwei.GetCheck()==0&&m_gurumin.GetCheck()==0&&m_dino.GetCheck()==0&&m_br4.GetCheck()==0&&m_ed3.GetCheck()==0&&m_ed4.GetCheck()==0&&m_ed5.GetCheck()==0){randomf=0; break;}
					for(;;){
						...
						if(a==2 && m_ed6fc.GetCheck()){Citiran_FC a;
						...
						if(a==3 && m_ed6sc.GetCheck()){itiran a;
						...
						if(a==4 && m_yso.GetCheck()){Citiran_YSO a;
						...
						if(a==5 && m_ed6tc.GetCheck()){CED63rd a;
						...
						if(a==6 && m_zweiii.GetCheck()){CZWEIII a;
						...
						if(a==7 && m_ysc1.GetCheck()){CYsC1 a;
						...
						if(a==8 && m_ysc2.GetCheck()){CYsC2 a;
						...
						if(a==10 && m_ys121.GetCheck()){CYs12_1 a;
						...
						if(a==11 && m_ys122.GetCheck()){CYs12_2 a;
							_getcwd( buffer, _MAX_DIR );
						...
						if(a==13 && m_zwei.GetCheck()){CZwei a;
						...
						if(a==14 && m_gurumin.GetCheck()){CGurumin a;
						...
						if(a==15 && m_dino.GetCheck()){CDino a;
...
//				fnn="順次演奏中";
				...
				for(;;){
					...
					CString ex;
					char buffer[_MAX_DIR];
					if(randomno<30+1) {if(!m_ys6.GetCheck())continue;
						b=randomno;filen.Format(_T("Ys6_%02d.ogg"),b);Citiran_YS6 a;a.Gett(b-1);
						modesub=4;ret2=b;b=m_dou.GetCheck();m_dou.SetCheck(0);playf=1;play();m_dou.SetCheck(b);
						break;
					}else
					if(randomno<34+30+1){if(!m_ysf.GetCheck())continue;
					...
					if(randomno<56+34+30+1){if(!m_ed6fc.GetCheck())continue; Citiran_FC a;
						b=randomno-34-31;s=a.Gett(b);filen.Format(_T("ED6%s.ogg"),s.Left(3));
						modesub=2;ret2=b;b=m_dou.GetCheck();m_dou.SetCheck(0);playf=1;play();m_dou.SetCheck(b);
						break;
					}else
					if(randomno<98+56+34+30+1){if(!m_ed6sc.GetCheck())continue; itiran a;
						b=randomno-56-34-30-1;s=a.Gett(b);filen.Format(_T("ED6%s.ogg"),s.Left(3));
						modesub=1;ret2=b;b=m_dou.GetCheck();m_dou.SetCheck(0);playf=1;play();m_dou.SetCheck(b);
						break;
					}else
					if(randomno<40+98+56+34+30+1){if(!m_yso.GetCheck())continue; Citiran_YSO a;
						b=randomno-98-56-34-30-1;s=a.Gett(b);filen.Format(_T("YSO_%s.ogg"),s.Left(3));
						modesub=5;ret2=b;b=m_dou.GetCheck();m_dou.SetCheck(0);playf=1;play();m_dou.SetCheck(b);
						break;
					}else
					if(randomno<141+40+98+56+34+30+1){if(!m_ed6tc.GetCheck())continue; CED63rd a;
						b=randomno-40-98-56-34-30-1;s=a.Gett(b);filen.Format(_T("ED6%s.ogg"),s.Left(3));
						modesub=6;ret2=b;b=m_dou.GetCheck();m_dou.SetCheck(0);playf=1;play();m_dou.SetCheck(b);
						break;
					}else
					if(randomno<65+141+40+98+56+34+30+1){if(!m_zweiii.GetCheck())continue; CZWEIII a;
						b=randomno-141-40-98-56-34-30-1;s=a.Gett(b);filen.Format(_T("ZW2_%s.ogg"),s.Left(3));
						modesub=7;ret2=b;b=m_dou.GetCheck();m_dou.SetCheck(0);playf=1;play();m_dou.SetCheck(b);
						break;
					}else
					if(randomno<72+65+141+40+98+56+34+30+1){if(!m_ysc1.GetCheck())continue; CYsC1 a;
						b=randomno-65-141-40-98-56-34-30-1;s=a.Gett(b);filen.Format(_T("%s.ogg"),s);
						modesub=8;ret2=b;b=m_dou.GetCheck();m_dou.SetCheck(0);playf=1;play();m_dou.SetCheck(b);
						break;
					}else
					if(randomno<92+72+65+141+40+98+56+34+30+1){if(!m_ysc2.GetCheck())continue; CYsC2 a;
						b=randomno-72-65-141-40-98-56-34-30-1;s=a.Gett(b);filen.Format(_T("%s.ogg"),s);
						modesub=9;ret2=b;b=m_dou.GetCheck();m_dou.SetCheck(0);playf=1;play();m_dou.SetCheck(b);
						break;
					}else
					if(randomno<24+92+72+65+141+40+98+56+34+30+1){if(!m_xa.GetCheck())continue; CXA a;
					...
					if(randomno<24+24+92+72+65+141+40+98+56+34+30+1){if(!m_ys121.GetCheck())continue; CYs12_1 a;
						_getcwd( buffer, _MAX_DIR );
						_tchdir(savedata.ys12);
						if(_chdir("wave\\wave_44")==-1){
							if(_chdir("wave\\wave_22")==-1){break;}
					...
					if(randomno<31+24+24+92+72+65+141+40+98+56+34+30+1){if(!m_ys122.GetCheck())continue; CYs12_2 a;
						_getcwd( buffer, _MAX_DIR );
						_tchdir(savedata.ys122);
						if(_chdir("wave\\wave_44")==-1){
							if(_chdir("wave\\wave_22")==-1){break;}
					...
					if(randomno<76+31+24+24+92+72+65+141+40+98+56+34+30+1){if(!m_sor.GetCheck())continue; CSor a;
						char buffer[_MAX_DIR];
						_getcwd( buffer, _MAX_DIR );
						_tchdir(savedata.sor);
						if(_chdir("WAVE\\WAVE44")==-1){
					...
					if(randomno<36+76+31+24+24+92+72+65+141+40+98+56+34+30+1){if(!m_zwei.GetCheck())continue; CZwei a;
						b=randomno-76-31-24-24-92-72-65-141-40-98-56-34-30-1;s=a.Gett(b);filen.Format(_T("%s(wav.dat)"),s);
						modesub=14;ret2=b;b=m_dou.GetCheck();m_dou.SetCheck(0);playf=1;play();m_dou.SetCheck(b);
						break;
					}else
					if(randomno<39+36+76+31+24+24+92+72+65+141+40+98+56+34+30+1){if(!m_gurumin.GetCheck())continue; CGurumin a;
					...
					if(randomno<33+39+36+76+31+24+24+92+72+65+141+40+98+56+34+30+1){if(!m_dino.GetCheck())continue; CDino a;
					...
					if(randomno<42+33+39+36+76+31+24+24+92+72+65+141+40+98+56+34+30+1){if(!m_br4.GetCheck())continue; CBr4 a;
						b=randomno-33-39-36-76-31-24-24-92-72-65-141-40-98-56-34-30-1;s=a.Gett(b);filen.Format(_T("%s.wav"),s);
						modesub=17;ret2=b;b=m_dou.GetCheck();m_dou.SetCheck(0);playf=1;play();m_dou.SetCheck(b);
						break;
					}else
					if(randomno<67+42+33+39+36+76+31+24+24+92+72+65+141+40+98+56+34+30+1){if(!m_ed3.GetCheck())continue; CED3 a;
						b=randomno-42-33-39-36-76-31-24-24-92-72-65-141-40-98-56-34-30-1;s=a.Gett(b);filen.Format(_T("%s.wav"),s);
						modesub=18;ret2=b;b=m_dou.GetCheck();m_dou.SetCheck(0);playf=1;play();m_dou.SetCheck(b);
						break;
					}else
					if(randomno<66+67+42+33+39+36+76+31+24+24+92+72+65+141+40+98+56+34+30+1){if(!m_ed4.GetCheck())continue; CED4 a;
					...
					if(randomno<98+66+67+42+33+39+36+76+31+24+24+92+72+65+141+40+98+56+34+30+1){if(!m_ed5.GetCheck())continue; CED5 a;
	...
	ret=_tchdir(savedata.san1);
		if(a.ret=="★"){
			...
			if(f.Open(filen,CFile::modeRead,NULL)==0){
				...
				if(f.Open(filen,CFile::modeRead,NULL)==0){fnn="ファイル又はフォルダがありません";return;}else f.Close();
			}else f.Close();
		}
		ret2=a.ret2;
		modesub=-14;
...
//	dc.FillSolidRect(0,0,300,100,RGB(0,0,0));
	for(i=0; i<88; i++) {
		...
		if(m_st.GetCheck()==FALSE){
			...
			dc.FillSolidRect(22*8+i*2,96-spelv[i],2,spelv[i]+1,RGB(0,128,0));
			dc.FillSolidRect(22*8+i*2,96-d       ,2,d+1       ,RGB(0,255,0));
			dc.FillSolidRect(22*8+i*2,96-spelv[i],2,1         ,RGB(0,255,0));
		}else{
			for(j=logtbl[i]; j<logtbl[i+1]; j++){
				if(dt  < ABS(aFFT2[j]))   dt = ABS(aFFT2[j]); // 区間内の最大値を採用
			...
			dc.FillSolidRect(22*8+i,96-spelv[100+i],1,spelv[100+i]+1,RGB(0,128,0));
			dc.FillSolidRect(22*8+i,96-d,1,d+1,RGB(0,255,0));
			if(dta>0) d = (int)( log10( dta* dta) * (80/5) );
			if( d > 80 ) d = 80; else if( d < 0 ) d = 0;
			if(spelv[200+i]<d){spelv[200+i]=d;spetm[200+i]=0;}
			dc.FillSolidRect(22*8+89+i,96-spelv[200+i],1,spelv[200+i]+1,RGB(0,128,0));
			dc.FillSolidRect(22*8+89+i,96-d,1,d+1,RGB(0,255,0));

			dc.FillSolidRect(22*8+i,96-spelv[100+i],1,1,RGB(0,255,0));
			dc.FillSolidRect(22*8+89+i,96-spelv[200+i],1,1,RGB(0,255,0));

			dc.FillSolidRect(22*8+88,16,1,81,RGB(0,255,255));
		}
	}
}
...
int COggDlg::mojisub(CString s,int x,int y,COLORREF rgb)
{
	...
	if(szinfo.cx<MDC+8)
		...
		dcsub.FillSolidRect(0,0,szinfo.cx+MDC+8,30,RGB(0,0,0));
    dcsub.TextOut(x, y ,s, s.GetLength());
...
//						playb-=wavbit*40;
   ...
   if(nSBCode==SB_PAGERIGHT){
		...
		case IDC_SLIDER2:
			...
			info=r->GetLineSize();
			curpos = r->GetPos();
			if (curpos < maxpos){curpos = min(maxpos, curpos + info);}else {hsc=2;return;}//
//			playb=curpos;
			if(pMediaPosition && (mode==-2 || (mode>0 && videoonly==TRUE))){
				r->SetPos((int)curpos);
				if(aa2==0){
...
void plus2(int &c)
{
	CString ss=sswk;
					...
					pFunck[kpicnt]=(pfnGetKMPModule)GetProcAddress(hDLLk1[kpicnt],"kmp_GetTestModule");
					if(pFunck[kpicnt]){
						{
							...
							for(int i=0;;i++){
								...
								if(mod1[kpicnt]->ppszSupportExts){
									if(mod1[kpicnt]->ppszSupportExts[i]==NULL||
									   mod1[kpicnt]->ppszSupportExts[i][0]==NULL){ext[kpicnt][i]==""; break;}
		...
		for(;h;){
			...
			if(!(ss =="." || ss == "..")){
					if(cf1.IsDirectory()!=0){ //フォルダ？
						if(ff.Right(1)=="\\")
							dir[cdd]=ff+cf1.GetFileName();
						else
							dir[cdd]=ff+_T("\\")+cf1.GetFileName();
						cdd++;
					}
			}
		}
